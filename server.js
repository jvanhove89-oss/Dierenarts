...
Vandaag is het ${vandaag}. De praktijk is in Tessenderlo, België.

Zoek naar het MEEST RECENTE nieuws (laatste 2 weken) uit:
- dgz.be (Dierengezondheidszorg Vlaanderen)
- amcra.be (antibioticabeleid)
- vilt.be (landbouw en dier)
- favv.be (voedselveiligheid)
- SAVAB (Vlaamse kleine dierenartsen)
- VetRecord, Veterinary Times, JAVMA (internationale vakpers)
- veterinaire fora en communities

Geef een PROFESSIONEEL overzicht voor dierenartsen. GEEN basisinfo die elke dierenarts al kent.
Focus op: actuele uitbraken/waarschuwingen in België, nieuwe regelgeving, klinisch afwijkende trends, sectorontwikkelingen.

Antwoord ENKEL in dit JSON-formaat:
{
  "gegenereerd_op": "${vandaag}",
  "bronnen_geraadpleegd": ["bron1", "bron2"],
  "secties": [
    {
      "type": "waarschuwing|nieuws|klinisch|regelgeving|sector",
      "urgentie": "hoog|medium|laag",
      "titel": "Titel max 10 woorden",
      "inhoud": "2-3 zinnen. Concreet, professioneel, geen basisinfo.",
      "bron": "Bronnaam",
      "bron_url": "url of leeg",
      "datum": "datum of leeg"
    }
  ],
  "klinische_radar": {
    "omschrijving": "1 zin: wat wijkt af van normaal voor ${maand} ${jaar}?",
    "items": [
      {"aandoening": "naam", "trend": "stijgend|dalend|ongewoon", "reden": "korte onderbouwing"}
    ]
  },
  "weer_context": {
    "situatie": "Huidig weer Tessenderlo en klinische relevantie deze week",
    "relevant_voor": ["aandoening1", "aandoening2"]
  }
}`;
  ...
  try {
    ...
    // Verzamel tekst uit content blocks
    const tekst = (result.content || [])
...
?/g, '').trim();
    ...
    if (startIdx === -1) throw new Error('Geen JSON in antwoord');
